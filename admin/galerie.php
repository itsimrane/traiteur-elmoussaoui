  ...
  <div class="container">
    ...
    <div class="videos-grid">
      <?php foreach ($videos as $v):
        ...
        if (!empty($v['url_video'])) {
          // Lien YouTube / Vimeo
          $embedSrc = $v['url_video'];
          if (!$thumbSrc && preg_match('/(?:youtu\.be\/|youtube\.com\/(?:watch\?v=|embed\/|shorts\/))([A-Za-z0-9_-]{6,})/', $v['url_video'], $m)) {
            $thumbSrc = 'https://img.youtube.com/vi/' . $m[1] . '/hqdefault.jpg';
          }
        } elseif ($v['fichier']) {
          // Fichier vidéo uploadé directement
          $embedSrc = UPLOAD_URL . '/' . $v['fichier'];
        }
        $titre    = htmlspecialchars($v['titre'] ?? 'Vidéo');
        $catNom   = htmlspecialchars($v['cat_nom'] ?? '');
      ?>
      <div class="video-card" data-aos="fade-up">
        <div class="video-thumb" onclick="playVideo('<?= addslashes($embedSrc) ?>')">
...
<div class="lightbox" id="videoModal">
  <div class="lightbox-inner">
    ...
    <video id="videoPlayer" class="lightbox-video" controls playsinline style="display:none"></video>
    <iframe id="videoEmbed" class="lightbox-video" style="display:none;aspect-ratio:16/9;border:none"
...
function toEmbedUrl(src) {
  // YouTube
  let m = src.match(/(?:youtu\.be\/|youtube\.com\/(?:watch\?v=|embed\/|shorts\/))([A-Za-z0-9_-]{6,})/);
...
function playVideo(src){
  ...
  const f = document.getElementById('videoEmbed');
  const embedUrl = toEmbedUrl(src);
  if (embedUrl) {
    // Lien YouTube / Vimeo
    p.style.display = 'none'; p.pause(); p.src = '';
    f.style.display = 'block'; f.src = embedUrl;
  } else {
    // Fichier vidéo uploadé directement
    f.style.display = 'none'; f.src = '';
    p.style.display = 'block'; p.src = src; p.play();
  }
  document.getElementById('videoModal').classList.add('open');
  document.body.style.overflow='hidden';
}
function closeVideo(){
  const p=document.getElementById('videoPlayer'); p.pause(); p.src='';
  const f=document.getElementById('videoEmbed'); f.src='';
