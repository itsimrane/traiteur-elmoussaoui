...
function genNotifications(PDO $pdo): array
{
  ...
  try {
    $resas = $pdo->query("
            SELECT r.id, c.prenom, c.nom, te.nom AS type_evenement, r.date_evenement, r.created_at
            FROM reservations r
  ...
  // Note : les paiements n'ont pas de notion de statut "en attente" —
  // un paiement enregistré est déjà un fait accompli, donc pas de
  // catégorie de notification pour ça.
    ...
    <main class="admin-main">
      ...
      <div class="admin-content">
        ...
        <?php if (empty($notifs)): ?>
          <!-- Tout OK -->
          <div
            style="text-align:center;padding:80px 20px;background:var(--dark-card);border:1px solid var(--border);border-radius:var(--radius)">
            <div
              style="width:80px;height:80px;border-radius:50%;background:rgba(37,211,102,.1);display:flex;align-items:center;justify-content:center;margin:0 auto 20px;font-size:2rem;color:#25D366">
          ...
          <div class="notif-layout">
            ...
            <div class="notif-sidebar">
              ...
              <div style="padding:12px 16px;border-top:1px solid var(--border);margin-top:4px">
                <div style="font-size:.7rem;color:#555;text-transform:uppercase;letter-spacing:.5px;margin-bottom:8px">
                  Accès rapide</div>
                <a href="messages.php"
                  style="display:flex;align-items:center;gap:8px;padding:6px 0;font-size:.78rem;color:var(--text-muted);text-decoration:none;transition:.2s"
            ...
            <div class="notif-list" id="notifList">
              <div class="notif-list-header">
                ...
                <span style="font-size:.75rem;color:var(--text-muted)"><?= $total ?>
                  notification<?= $total > 1 ? 's' : '' ?></span>
              </div>
