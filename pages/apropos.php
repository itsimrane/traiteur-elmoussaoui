...
$statVilles = 0;
try {
  $statEvenements = (int) $pdo->query("SELECT COUNT(*) FROM devis_generes")->fetchColumn();
} catch (Exception $e) {
}
try {
  $statClients = (int) $pdo->query("SELECT COUNT(*) FROM clients")->fetchColumn();
} catch (Exception $e) {
}
try {
...
try {
  ...
  if ($anneeFondation > 1990)
    $anneesExperience = max(1, (int) date('Y') - $anneeFondation);
} catch (Exception $e) {
}
...
$equipe = [];
try {
  $equipe = $pdo->query("SELECT * FROM equipe WHERE actif=1 ORDER BY ordre ASC, id ASC")->fetchAll();
} catch (Exception $e) {
}
...
$timeline = [];
try {
  $timeline = $pdo->query("SELECT * FROM apropos_timeline WHERE actif=1 ORDER BY ordre ASC, annee ASC")->fetchAll();
} catch (Exception $e) {
}
...
<head>
  ...
  <meta name="description"
    content="Découvrez l'histoire de Traiteur EL MOUSSAOUI, expert en organisation d'événements à Errachidia depuis plus de 10 ans.">
  ...
  <style>
    ...
    .story-img {
      ...
      background-image: linear-gradient(135deg, #1A1200, #0D0900);
      position: relative;
      overflow: hidden;
      display: flex;
      align-items: center;
    ...
    /* Stats */
    .stats-section {
      background: linear-gradient(135deg, #0D0900, #050300);
      padding: 70px 0;
    }
    ...
    .team-section {
      padding: 90px 0;
      background: linear-gradient(180deg, #0D0900 0%, var(--dark) 100%);
    }
    ...
    .team-avatar {
      height: 140px;
      background: linear-gradient(135deg, #1A1200, var(--dark-3));
      display: flex;
      align-items: center;
      justify-content: center;
      font-size: 3rem;
    ...
    .tl-item:nth-child(even) .tl-year-wrap {
      text-align: left;
    }
...
<body>
  ...
  <div class="page-hero">
    <div class="container">
      <span class="section-tag" data-aos="fade-down" data-fr="Notre histoire" data-ar="قصتنا">Notre histoire</span>
      <h1 data-aos="fade-up" data-fr="À Propos de Nous" data-ar="من نحن">À Propos de Nous</h1>
      <p data-aos="fade-up" data-aos-delay="200"
        ...
        Depuis plus de 10 ans, nous mettons notre passion et notre expertise au service de vos moments les plus précieux
        à Errachidia et dans la région.
      </p>
    </div>
  </div>
    ...
    <div class="container">
      <div class="story-grid">
        ...
        <div data-aos="fade-left">
          <span class="section-tag" data-fr="Notre histoire" data-ar="قصتنا">Notre histoire</span>
          <h2 style="margin-top:12px" data-fr="L'Art de Créer des Souvenirs Inoubliables"
          ...
          <p data-fr="Traiteur EL MOUSSAOUI est né d'une passion profonde pour l'art culinaire marocain et l'organisation d'événements. Fondé en 2015 à Errachidia, nous avons bâti notre réputation sur la qualité, la générosité et l'attention portée aux moindres détails."
            ...
            Traiteur EL MOUSSAOUI est né d'une passion profonde pour l'art culinaire marocain et l'organisation
            d'événements. Fondé en 2015 à Errachidia, nous avons bâti notre réputation sur la qualité, la générosité et
          ...
          <p data-fr="Chaque événement est unique pour nous. Qu'il s'agisse d'un mariage grandiose, de fiançailles intimes ou d'une circoncision en famille, nous apportons le même niveau d'excellence et de professionnalisme à chaque prestation."
            ...
            Chaque événement est unique pour nous. Qu'il s'agisse d'un mariage grandiose, de fiançailles intimes ou
            d'une circoncision en famille, nous apportons le même niveau d'excellence et de professionnalisme.
          </p>
          ...
          <div class="story-values">
            <div class="story-val">
              ...
              <p data-fr="Chaque plat est préparé avec amour et savoir-faire traditionnel."
                data-ar="كل طبق يُعدّ بحب ومهارة تقليدية.">Chaque plat est préparé avec amour et savoir-faire
                traditionnel.</p>
            </div>
            <div class="story-val">
              ...
              <p data-fr="Des standards élevés à chaque étape de votre événement."
                data-ar="معايير عالية في كل مرحلة من مراحل مناسبتك.">Des standards élevés à chaque étape de votre
                événement.</p>
            </div>
            <div class="story-val">
              ...
              <p data-fr="Plus de 500 familles nous font confiance depuis 10 ans."
                data-ar="أكثر من 500 عائلة تثق بنا منذ 10 سنوات.">Plus de 500 familles nous font confiance depuis 10
                ans.</p>
            </div>
            <div class="story-val">
              <i class="fas fa-map-marker-alt"></i>
              <h4 data-fr="Local & Régional" data-ar="محلي وإقليمي">Local & Régional</h4>
              <p data-fr="Présents dans toute la région de Drâa-Tafilalet." data-ar="نغطي منطقة درعة تافيلالت بأكملها.">
                Présents dans toute la région de Drâa-Tafilalet.</p>
            </div>
          </div>
        </div>
      </div>
  ...
  $statsToShow = [];
  if ($statEvenements > 0)
    $statsToShow[] = ['num' => $statEvenements, 'unit' => '+', 'fr' => 'Événements traités', 'ar' => 'مناسبة'];
  if ($anneesExperience > 0)
    $statsToShow[] = ['num' => $anneesExperience, 'unit' => '+', 'fr' => "Années d'expérience", 'ar' => 'سنوات من الخبرة'];
  if ($statNote > 0)
    $statsToShow[] = ['num' => $statNote, 'unit' => '/5', 'fr' => 'Note moyenne clients', 'ar' => 'متوسط تقييم العملاء'];
  if ($statVilles > 0)
    $statsToShow[] = ['num' => $statVilles, 'unit' => '+', 'fr' => 'Villes couvertes', 'ar' => 'مدينة مغطاة'];
  ?>
        ...
        <div class="stats-row">
          <?php foreach ($statsToShow as $i => $s): ?>
            <div class="stat-block" data-aos="fade-up" data-aos-delay="<?= $i * 80 ?>">
              <div><span class="big-num" dir="ltr"><?= htmlspecialchars($s['num']) ?></span><span
                  class="stat-unit"><?= $s['unit'] ?></span></div>
    ...
    <section class="team-section">
      <div class="container">
        <div class="section-header" data-aos="fade-up" style="margin-bottom:48px">
          <span class="section-tag light" data-fr="Les personnes derrière la magie" data-ar="الأشخاص خلف السحر">Les
            personnes derrière la magie</span>
          <h2 class="section-title light" data-fr="Notre Équipe" data-ar="فريقنا">Notre Équipe</h2>
        </div>
        <div class="team-grid">
          <?php foreach ($equipe as $i => $m): ?>
            <div class="team-card" data-aos="fade-up" data-aos-delay="<?= ($i % 4) * 80 ?>">
              ...
              <div class="team-body">
                ...
                <div class="team-role" data-fr="<?= htmlspecialchars($m['role']) ?>"
                  data-ar="<?= htmlspecialchars($m['role_ar'] ?: $m['role']) ?>"><?= htmlspecialchars($m['role']) ?></div>
              </div>
            </div>
          <?php endforeach; ?>
        </div>
    ...
    <section class="timeline-section">
      <div class="container">
        <div class="section-header" data-aos="fade-up" style="margin-bottom:56px">
          <span class="section-tag light" data-fr="Notre parcours" data-ar="مسيرتنا">Notre parcours</span>
          <h2 class="section-title" data-fr="Notre Histoire en Dates" data-ar="تاريخنا عبر السنين">Notre Histoire en Dates
          </h2>
        </div>
        <div class="timeline">
          <?php foreach ($timeline as $i => $j): ?>
            <div class="tl-item" data-aos="<?= $i % 2 === 0 ? 'fade-right' : 'fade-left' ?>">
              ...
              <div class="tl-content">
                <h4 data-fr="<?= htmlspecialchars($j['titre']) ?>"
                  data-ar="<?= htmlspecialchars($j['titre_ar'] ?: $j['titre']) ?>"><?= htmlspecialchars($j['titre']) ?></h4>
                <?php if (!empty($j['description'])): ?>
                  <p data-fr="<?= htmlspecialchars($j['description']) ?>"
                    data-ar="<?= htmlspecialchars($j['description_ar'] ?: $j['description']) ?>">
    ...
    <section class="section" id="temoignages">
      <div class="container">
        ...
        <div class="testimonials-grid"
          style="display:grid;grid-template-columns:repeat(auto-fit,minmax(300px,1fr));gap:24px;margin-top:32px">
          <?php foreach ($tousTemoignages as $t):
            ...
            <div class="testimonial-card" data-aos="fade-up">
              ...
              <div class="testi-author">
                ...
                <div><strong><?= htmlspecialchars($t['nom_client']) ?></strong><span><?= htmlspecialchars($lieu) ?></span>
                </div>
              </div>
            </div>
          <?php endforeach; ?>
  ...
  <section class="section-cta">
    ...
    <div class="container">
      <div class="cta-content" data-aos="zoom-in">
        <h2 data-fr="Rejoignez notre famille<br>de clients satisfaits" data-ar="انضم إلى عائلتنا<br>من العملاء الراضين"
          data-html>
